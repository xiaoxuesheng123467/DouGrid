...
当前版本为 `1.2.0`，支持 Android 8.0 及以上。App 不申请网络权限，图片识别全程离线完成，项目、参考图和库存数据只保存在设备本机。
...
- 识图时自动匹配品牌豆子型号并统计颗数，生成后直接给出库存缺口与购买袋数，无需手填
- OKLab 感知颜色匹配、受控减色、可调抖动、杂点清理与边缘保留
- MARD 221/291、COCO 291 和 Artkal C197 品牌色卡
- 画笔和橡皮实时落豆，支持填充、吸色、同色替换、镜像、整笔撤销/重做和参考图临摹
